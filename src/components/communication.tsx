...
const chatHistories: { [key: number]: ChatMessage[] } = {
  1: [
    { from: 'me', text: 'Train 12926 approaching your section. Confirm track is clear.' },
    { from: 'them', text: 'Track confirmed clear for 12926. Proceed.' },
  ],
  2: [
    { from: 'me', text: 'Any updates on the signal at MP 255.5?' },
  ],
  3: [
    { from: 'them', text: 'Offline. Last seen 2 hours ago.' },
  ],
};
...
export default function CommunicationGateway() {
  ...
  const [selectedController, setSelectedController] = useState<Controller>(adjacentControllers[0]);
  ...
  return (
    <aside className="bg-gray-800 text-gray-300 w-96 p-4 flex flex-col flex-shrink-0 border-l border-gray-700 h-full">
      ...
      <div className="flex justify-between items-center mb-4 px-2 flex-shrink-0">
        <h2 className="text-xl font-bold text-white tracking-tight">Communication</h2>
                    ...
                    <li key={controller.id}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`relative w-7 h-7 rounded-full bg-gray-600 flex items-center justify-center text-xs font-bold text-white`}>
                            {controller.name.charAt(0)}
                            <span className={`absolute bottom-0 right-0 block h-2 w-2 rounded-full ring-1 ring-gray-700 ${controller.status === 'online' ? 'bg-green-500' : 'bg-gray-500'}`} />
                          </div>
                          <div>
                            <p className="font-semibold text-white text-sm">{controller.name}</p>
                            <p className="text-xs text-gray-400">{controller.section}</p>
                          </div>
                        </div>
                      </button>
                    </li>
      ...
      <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-3 flex flex-col flex-grow min-h-60">
        ...
        <div className="flex-grow space-y-3 text-sm overflow-y-auto pr-2 mb-3">
          ...
          {chatHistories[selectedController.id]?.map((chat: ChatMessage, index: number) => (
            <div key={index} className={`text-gray-300 ${chat.from === 'them' ? 'text-right' : ''}`}>
              <p className={`p-2 rounded-lg inline-block ${chat.from === 'me' ? 'bg-blue-600/50' : 'bg-gray-700'}`}>
                {chat.text}
              </p>
            </div>
          ))}
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <input type="text" placeholder="Type a message..." className="flex-grow bg-gray-700 border border-gray-600 text-gray-200 text-sm rounded-lg p-2 focus:ring-sky-500 focus:border-sky-500" />
